<x-admin-layout title="Edit Aturan Fuzzy">
    <div class="mb-8 flex items-center justify-between">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="text-2xl font-bold text-slate-800">Edit Aturan Fuzzy</h2>
                <p class="text-slate-500 text-sm mt-1">Perbarui logika inferensi untuk aturan R{{ sprintf('%03d', $fuzzy_rule->rule_number) }}.</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-[2rem] border border-slate-100 shadow-sm overflow-hidden">
        <form action="{{ route('admin.fuzzy-rules.update', $fuzzy_rule->id) }}" method="POST" class="p-8">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                <div class="space-y-6">
                    <div>
                        ...
                        <input type="number" name="rule_number" value="{{ old('rule_number', $fuzzy_rule->rule_number) }}" required readonly
                               class="w-full px-4 py-3 bg-slate-100 border border-slate-200 rounded-xl text-slate-500 cursor-not-allowed text-sm">
                    </div>
                    ...
                    <div class="p-5 bg-slate-50 rounded-xl border border-slate-100 space-y-4">
                        ...
                        <div>
                            ...
                            <select name="antecedent_total"
                                ...
                                <option value="minimal" {{ old('antecedent_total', $fuzzy_rule->antecedent_total?->value) === 'minimal' ? 'selected' : '' }}>Minimal</option>
                                <option value="ringan" {{ old('antecedent_total', $fuzzy_rule->antecedent_total?->value) === 'ringan' ? 'selected' : '' }}>Ringan</option>
                                <option value="sedang" {{ old('antecedent_total', $fuzzy_rule->antecedent_total?->value) === 'sedang' ? 'selected' : '' }}>Sedang</option>
                                <option value="berat" {{ old('antecedent_total', $fuzzy_rule->antecedent_total?->value) === 'berat' ? 'selected' : '' }}>Berat</option>
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <select name="antecedent_cognitive"
                                    class="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl focus:outline-none focus:border-[#0d7a70] focus:ring-2 focus:ring-[#0d7a70]/20 transition-all text-sm">
                                <option value="">(Abaikan / None)</option>
                                <option value="minimal" {{ old('antecedent_cognitive', $fuzzy_rule->antecedent_cognitive?->value) === 'minimal' ? 'selected' : '' }}>Minimal</option>
                                <option value="ringan" {{ old('antecedent_cognitive', $fuzzy_rule->antecedent_cognitive?->value) === 'ringan' ? 'selected' : '' }}>Ringan</option>
                                <option value="sedang" {{ old('antecedent_cognitive', $fuzzy_rule->antecedent_cognitive?->value) === 'sedang' ? 'selected' : '' }}>Sedang</option>
                                <option value="berat" {{ old('antecedent_cognitive', $fuzzy_rule->antecedent_cognitive?->value) === 'berat' ? 'selected' : '' }}>Berat</option>
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <select name="antecedent_somatic"
                                    class="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl focus:outline-none focus:border-[#0d7a70] focus:ring-2 focus:ring-[#0d7a70]/20 transition-all text-sm">
                                <option value="">(Abaikan / None)</option>
                                <option value="minimal" {{ old('antecedent_somatic', $fuzzy_rule->antecedent_somatic?->value) === 'minimal' ? 'selected' : '' }}>Minimal</option>
                                <option value="ringan" {{ old('antecedent_somatic', $fuzzy_rule->antecedent_somatic?->value) === 'ringan' ? 'selected' : '' }}>Ringan</option>
                                <option value="sedang" {{ old('antecedent_somatic', $fuzzy_rule->antecedent_somatic?->value) === 'sedang' ? 'selected' : '' }}>Sedang</option>
                                <option value="berat" {{ old('antecedent_somatic', $fuzzy_rule->antecedent_somatic?->value) === 'berat' ? 'selected' : '' }}>Berat</option>
                            </select>
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-6">
                    <div class="p-5 bg-emerald-50 rounded-xl border border-emerald-100">
                        ...
                        <div>
                            ...
                            <select name="consequent" required
                                ...
                                <option value="minimal" {{ old('consequent', $fuzzy_rule->consequent?->value) === 'minimal' ? 'selected' : '' }}>Minimal</option>
                                <option value="ringan" {{ old('consequent', $fuzzy_rule->consequent?->value) === 'ringan' ? 'selected' : '' }}>Ringan</option>
                                <option value="sedang" {{ old('consequent', $fuzzy_rule->consequent?->value) === 'sedang' ? 'selected' : '' }}>Sedang</option>
                                <option value="berat" {{ old('consequent', $fuzzy_rule->consequent?->value) === 'berat' ? 'selected' : '' }}>Berat</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="p-5 border border-slate-100 rounded-xl">
                        <label class="flex items-center gap-3 cursor-pointer">
                            <input type="checkbox" name="is_active" value="1" {{ old('is_active', $fuzzy_rule->is_active) ? 'checked' : '' }}
